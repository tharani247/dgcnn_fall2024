...
def load_data_partseg(partition):
    ...
    # Add debugging print statements here to inspect the process
    print("Data directories/files being loaded:", 'data/shapenet_part_seg_hdf5_data/shapenet_c')
    print("Contents of all_data before concatenation:", all_data)
    if len(all_data) == 0:
        print("Warning: No data loaded. Please check the dataset path.")
    return all_data
...
def train(args, io):
    ...
    for epoch in range(args.epochs):
        ...
        if np.mean(test_ious) >= best_test_iou:
            ...
            torch.save(model.state_dict(), 'outputs/%s/models/model.t8' % args.exp_name)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--batch_size', type=int, default=128, metavar='batch_size',
                        help='Size of batch)')
    parser.add_argument('--test_batch_size', type=int, default=256, metavar='batch_size',
                        help='Size of batch)')
