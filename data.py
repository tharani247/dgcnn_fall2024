...
def download_shapenetpart():
    ...
    if not os.path.exists(DATA_DIR):
        os.mkdir(DATA_DIR)
    if not os.path.exists(os.path.join(DATA_DIR, 'shapenet_part_seg_hdf5_data/shapenet_c')):
        www = 'https://huggingface.co/datasets/ShapeNet/PartNet-archive/resolve/main/sem_seg_h5.zip?download=true'
        zipfile = os.path.basename(www)
        os.system('wget --no-check-certificate %s; unzip %s' % (www, zipfile))
        os.system('mv %s %s' % ('hdf5_data', os.path.join(DATA_DIR, 'shapenet_part_seg_hdf5_data/shapenet_c')))
        os.system('rm %s' % (zipfile))
...
def load_data_partseg(partition):
    ...
    if partition == 'trainval':
        file = glob.glob(os.path.join(DATA_DIR, 'shapenet_part_seg_hdf5_data/shapenet_c', '*train*.h5')) \
               + glob.glob(os.path.join(DATA_DIR, 'shapenet_part_seg_hdf5_data/shapenet_c', '*val*.h5'))
    else:
        file = glob.glob(os.path.join(DATA_DIR, 'shapenet_part_seg_hdf5_data/shapenet_c', '*%s*.h5'%partition))
    for h5_name in file:
        f = h5py.File(h5_name, 'r+')
        data = f['data'][:].astype('float32')
        label = f['label'][:].astype('int64')
    ...
    print(f"Looking for files in {DATA_DIR}/shapenet_part_seg_hdf5_data")
